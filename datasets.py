...
class kitti(io.Dataset):
    ...
    def __init__(
        ...
        if mode in ["train", "val"]:
            ...
            self.depths = list(
                sorted(glob.iglob(self.depth_path + "/**/*.png", recursive=True))
            )
            self.lidars = list(
                sorted(glob.iglob(self.lidar_path + "/**/*.png", recursive=True))
            )
        elif mode == "selval":
            self.depth_path = os.path.join(
                self.base_dir, "val_selection_cropped", "groundtruth_depth"
            )
            self.lidar_path = os.path.join(
                self.base_dir, "val_selection_cropped", "velodyne_raw"
            )
            self.image_path = os.path.join(
                self.base_dir, "val_selection_cropped", "image"
            )
            self.depths = list(
                sorted(glob.iglob(self.depth_path + "/*.png", recursive=True))
            )
            ...
            self.images = list(
                sorted(glob.iglob(self.image_path + "/*.png", recursive=True))
            )
        elif mode == "test":
            self.lidar_path = os.path.join(
                self.base_dir, "test_depth_completion_anonymous", "velodyne_raw"
            )
            self.image_path = os.path.join(
                self.base_dir, "test_depth_completion_anonymous", "image"
            )
            self.lidars = list(
                sorted(glob.iglob(self.lidar_path + "/*.png", recursive=True))
            )
    ...
    def pull_DEPTH(self, path):
        ...
        depth_image = (depth_png / 256.0).astype(np.float32)
        return depth_image
